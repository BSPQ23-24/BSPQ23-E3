...
const Register = ({ showLoginForm }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (userType == "User") {
      ...
      try {
        const response = await fetch(
          "http://localhost:8080/rest/host/register",
          {
            method: "POST",
            headers: {
              "Content-Type": "application/json",
        ...
        if (response.ok) {
          resetFields();
          setWarning("Host registered successfully!");
          console.log("Host registered successfully!");
        } else if (responseBody == "Host already exists!") {
          console.error(
            "Failed to register Host because the Host already exists"
          );
          setError("Host already exists!");
        } else if (responseBody == "Fill all the data!") {
          console.error(
            "Failed to register Host because all the data has not been filled"
          );
          setError("Fill all the data!");
        } else if (responseBody == "Not valid phone number!") {
          console.error(
            "Failed to register Host because the phone number must have 9 digits"
          );
          setError("Not valid phone number!");
        } else if (responseBody == "Not valid email!") {
          console.error("Failed to register Host because of email");
          setError("Not valid email!");
        } else {
          console.error(responseBody);
        }
