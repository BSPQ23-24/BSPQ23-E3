  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await fetch(
        "http://localhost:8080/rest/residence/register",
        {
          method: "POST",
          headers: {
  ...
  return (
    <div className="text-center">
      ...
      <h1 className="font-bold text-3xl mt-8">
        Hi, {user ? user.name : "Invitado"}!
      </h1>
      {user ? (
        user.user_type === "Host" ? (
          <div>
            <p className="font-semibold text-xl mt-8">
              As you are a host, you can add residences!
            </p>
            <div className="bg-gray-100 m-8 mb-8 mx-auto p-8 rounded-lg shadow-top text-center w-2/5">
              <form onSubmit={handleSubmit}>
                ...
                {warning && (
                  <div className="text-green-500 mb-2">{warning}</div>
                )}
                <div className="flex mt-4 justify-center">
                  <button
                    type="submit"
          ...
          <p className="font-semibold text-xl mt-8">
            Sorry, you are not a host, so you can't add any residence!
          </p>
        )
      ) : null}
    </div>
